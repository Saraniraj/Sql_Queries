...
#End Region
Public Class FrmMaterialGPassOUT_NotReturnable
    ...
    Private Sub FrmMaterialGatePassOUT_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            DtpDCDate.Value = Now
            RdlReturn.Checked = False
            RdlNotreturn.Checked = True
            RdlReturn.Visible = False
            RdlNotreturn.Visible = True
    ...
    Private Sub fill_Combobox_Details()
        Try
            ...
            Fill_CBX_details("  SELECT   distinct [To_Company]  FROM [DB_General].[dbo].[GEN_MaterialOUTNOTReturn_Main] order by  To_Company  ", "DB_General", CbxTOCompany)

            Fill_CBX_details("  SELECT   distinct [Vehicle_No]  FROM [DB_General].[dbo].[GEN_MaterialOUTNOTReturn_Main] order by  Vehicle_No        ", "DB_General", CbxMaterialfrom)

            Fill_CBX_details("  SELECT   distinct Prod_Code as Prod_Code  FROM [DB_General].[dbo].[GEN_MaterialOUTNOTReturn_Line] order by  Prod_Code        ", "DB_General", cbxProdDesc)
            ''distinct[]as

            Fill_CBX_details("  SELECT   distinct Delivered_By as Delivered_By  FROM [DB_General].[dbo].[GEN_MaterialOUTNOTReturn_Main] order by  Delivered_By        ", "DB_General", CbxDeliveredBy)

            'Fill_CBX_details("  SELECT   distinct UOM_Code as UOM_Code  FROM [DB_General].[dbo].[GEN_MaterialOUTNOTReturn_Line] order by  UOM_Code        ", "DB_General", CbxUOM)
    ...
    Private Sub Fill_Numbering_Type()
        Try
            ...
            ds = GetNumberTypeDetails("Material Gatepass OUT", 0)
            If ds.Tables(0).Rows.Count <= 0 Then Exit Sub
            With cbxNumberType
                .Items.Clear()
    ...
    Private Sub btnPK_Gatepass_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPK_Gatepass.Click
        Try
            ...
            dt = New DataTable
            dtPickList = Nothing
            ds = mobjclsGatepass.GetMaterialOUTNOTReturn_Details("", "", "", "ALL")
            dt = ds.Tables(0)
    ...
    Private Sub Fill_Screen_Details()
        Try
            ...
            ds = Nothing
            ds = New DataSet
            ds = mobjclsGatepass.GetMaterialOUTNOTReturn_Details(Trim(tbGatepassNo.Text), "", "", "SPECIFY")
            If ds.Tables(0).Rows.Count <= 0 Then Exit Sub
        ...
        Try
            ...
            If Not RemoveSingleQuotes(tbGatepassNo.Text) = "" Then
                mvarRecordStatus = GetDeleteAutorize_Status(RemoveSingleQuotes(tbGatepassNo.Text), Format(dtpGatepassDate.Value, "yyyy/MM/dd") _
                                                           , "Gpass_No", "Gpass_Date", "DB_General..GEN_MaterialOUTNOTReturn_Main", "AUTHORIZE")
                If mvarRecordStatus = 3 Then
                    Show_Message("Already authorized.")
                    Exit Sub
                End If
                mvarRecordStatus = GetDeleteAutorize_Status(RemoveSingleQuotes(tbGatepassNo.Text), Format(dtpGatepassDate.Value, "yyyy/MM/dd") _
                                                           , "Gpass_No", "Gpass_Date", "DB_General..GEN_MaterialOUTNOTReturn_Main", "DELETE")
                ...
                mvarRecordStatus = GetDeleteAutorize_Status(RemoveSingleQuotes(tbGatepassNo.Text), Format(dtpGatepassDate.Value, "yyyy/MM/dd") _
                                                           , "Gpass_No", "Gpass_Date", "DB_General..GEN_MaterialOUTNOTReturn_Main", "")
            End If
            ...
            Select Case cbxScreenMode.Text
                ...
                Case "Authorize"
                    ...
                    Else
                        mvarUpateStatus = updateDeleteAutorize_Status(RemoveSingleQuotes(tbGatepassNo.Text), Format(dtpGatepassDate.Value, "yyyy/MM/dd") _
                                                           , "Gpass_No", "Gpass_Date", "DB_General..GEN_MaterialOUTNOTReturn_Line", "AUTHORIZE")
                        mvarUpateStatus = updateDeleteAutorize_Status(RemoveSingleQuotes(tbGatepassNo.Text), Format(dtpGatepassDate.Value, "yyyy/MM/dd") _
                                                            , "Gpass_No", "Gpass_Date", "DB_General..GEN_MaterialOUTNOTReturn_Main", "AUTHORIZE")
                ...
                Case "Delete"
                    If mvarRecordStatus >= 1 Then
                        ...
                        If mvarMsgDialogResult = MsgBoxResult.Yes Then
                            mvarUpateStatus = updateDeleteAutorize_Status(RemoveSingleQuotes(tbGatepassNo.Text), Format(dtpGatepassDate.Value, "yyyy/MM/dd") _
                                                           , "Gpass_No", "Gpass_Date", "DB_General..GEN_MaterialOUTNOTReturn_Line", "DELETE")
                            mvarUpateStatus = updateDeleteAutorize_Status(RemoveSingleQuotes(tbGatepassNo.Text), Format(dtpGatepassDate.Value, "yyyy/MM/dd") _
                                                             , "Gpass_No", "Gpass_Date", "DB_General..GEN_MaterialOUTNOTReturn_Main", "DELETE")
            ...
            If Trim(tbGatepassNo.Text) = "" Then
                If Format(dtpGatepassDate.Value, "yyyy/MM/dd") < Format(GetMaxDate("GEN_MaterialOUTNOTReturn_Main", "Gpass_Date", "DB_General"), "yyyy/MM/dd") Then
            ...
            tbGatepassNo.Text = mobjclsGatepass.SaveGEN_MaterialOUTNOTReturn_Main(mSaveString, cbxScreenMode.Text, c1fgItems, cbxNumberType.Text)
    ...
    Private Sub GetMaxDCNO()
        Try
            ...
            SSQL &= "  from  [DB_General].[dbo].[GEN_MaterialOUTNOTReturn_Main]   "
    ...
    Private Sub GetGatepassPrint()
        ...
        Try
            ...
            If mvarLocationCode = "WEAVING" Or mvarLocationCode = "PALLADAM" Then
                ds = mobjclsGatepass.GetMaterialOUTNOTReturn_Details(Trim(tbGatepassNo.Text), "", "", "Gatepass_Print_Authorized")
                If ds.Tables(0).Rows.Count <= 0 Then
                    Show_Message("Kindly Authorize Entry or  record Missing.")
                    Exit Sub
                End If
            Else
                ds = mobjclsGatepass.GetMaterialOUTNOTReturn_Details(Trim(tbGatepassNo.Text), "", "", "Gatepass_Print_ALL")
