...
Public Class frmSTH_OperationMaster
    ...
    Public Sub New(ByVal id As Long, ByVal Action As Integer)
        InitializeComponent()
        FillScreenMode("STH Operation Master", cbxScreenMode)
        If Action = 1 Then
    ...
    Public Sub New()
        ...
        ''   loadexisting(id)
    End Sub
    Private Sub frmSTH_OperationMaster_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub LoadData()
        ' FillDataComboBox("SELECT STH_Prod_Code,Product_Name FROM STH_Product_Master", "STH_Prod_Code", "Product_Name", CbxProductname)
        FillDataGrid(" select  Ops_Code,Operation_Name,Product_Name,Rate_Per_Pcs,IS_Active FROM  STH_Operation_Master  order by Product_Name", DgvDisplay, mvarDbasename)
    ...
    Private Sub Fill_Combo_Details()
        ...
        Try
            ...
            ds = New DataSet
            SSQL = ""
            SSQL = "SELECT Product_Name,STH_Prod_Code FROM STH_Product_Master ORDER BY STH_Prod_Code"
            ds = ReturnMultipleValue(SSQL, mvarDbasename)
            If ds.Tables(0).Rows.Count <= 0 Then
                Show_Message("Please Enter  Product Master.")
            Else
    ...
    Public Sub LoadDG()
        Try
            ...
            dt = Nothing
            dt = New DataTable()
            ds = mobjRD_Stitching.Get_STH_Operationmaster_Details("", "")
            dt = ds.Tables(0)
    ...
    Private Sub btnApply_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnApply.Click
        ...
        Try
            ...
            If IsBlankTextBox(txtOperationName) Then Exit Sub
            ...
            Select Case cbxScreenMode.Text
                Case "Add"
                    If mvarRecordStatus >= 1 Then
                        mvarMsgDialogResult = ShowYesNo("STH Operation Master Details  details are already exists." _
                                                             & vbCrLf & "Do you want to modify?")
                        If mvarMsgDialogResult = MsgBoxResult.Yes Then
                            cbxScreenMode.Text = "Modify"
                        ElseIf mvarMsgDialogResult = MsgBoxResult.No Then
                            mvarMsgDialogResult = ShowYesNo("Do you want to create new STH Operation Master Details  ?")
                            If mvarMsgDialogResult = MsgBoxResult.Yes Then
                                lblId.Text = ""
                            Else
                                Exit Sub
                ...
                Case "Modify"
                    If mvarRecordStatus <= 0 Then
                        mvarMsgDialogResult = ShowYesNo("STH Operation Master Details are doesn't exist." _
                                                             & vbCrLf & "Do you want to create new STH Operation Master  Details?")
                        If mvarMsgDialogResult = MsgBoxResult.Yes Then
                            lblId.Text = ""
                            cbxScreenMode.Text = "Add"
                        Else
                ...
                Case "Authorize"
                    If mvarRecordStatus <= 0 Then
                        Show_Message("STH Operation Master  Details    details are doesn't exists.")
                        Exit Sub
                    Else
            ...
            mSaveString &= RemoveSingleQuotes(txtOperationName.Text) & "||"
            mSaveString &= RemoveSingleQuotes(CbxProductname.Text) & "||"
            mSaveString &= Val(TxtRate_Per_Pcs.Text) & "||"

            mSaveString &= misactive & "||"
            lblId.Text = mobjRD_Stitching.Save_STH_Operationmaster(mSaveString, cbxScreenMode.Text, "")
    ...
    Private Sub btnPickList_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPickList.Click
        Try
            ...
            frmPickList1.Text = "Operation Master Details"
            ...
            lblId.Text = dtPickList.Rows(0)("Ops_Code")
            Fill_Screen_Details()
        Catch ex As Exception
            Show_Message(ex.Message)
        End Try
    ...
    Public Sub Fill_Screen_Details()
        Try
            ...
            ds = Nothing
            ds = New DataSet
            ds = mobjRD_Stitching.Get_STH_Operationmaster_Details(Trim(lblId.Text), "")
            ...
            If ds.Tables(0).Rows.Count <= 0 Then
                ...
                Exit Sub
            End If
            txtOperationName.Text = CheckNullValues(ds.Tables(0).Rows(0)("Operation_Name"), 1)
            lblId.Text = CheckNullValues(ds.Tables(0).Rows(0)("Ops_Code"), 1)
            CbxProductname.Text = CheckNullValues(ds.Tables(0).Rows(0)("Product_Name"), 1)
            TxtRate_Per_Pcs.Text = CheckNullValues(ds.Tables(0).Rows(0)("Rate_Per_Pcs"), 1)
    ...
    Private Sub DGVDisplay_CellClick(ByVal sender As Object, ByVal e As DataGridViewCellEventArgs) Handles DgvDisplay.CellClick
        Try
            ...
            If (e.RowIndex <> -1) AndAlso (e.ColumnIndex <> -1) Then
                ...
                Me.txtOperationName.Text = Convert.ToString(Me.DgvDisplay.CurrentRow.Cells(1).Value)
